...
//        fieldArrayList.add(matriField);
    ...
    public void updateMember(ActionEvent event) throws FileNotFoundException {
        try {
            ...
            if (result <= 2) {
                message = "Les informations de " + membre.getNom() + "\n" + membre.getPrenom().toUpperCase() + " ont ete bien modifiees!";
                tray.setTitle(title);
        ...
        if (ButtonType.OK.equals(result.get())) {
            ...
            try {
                ...
                */
                ...
                if (1 == i) {
                    message = "Le Statut de " + membre.getNom() + "\n" + membre.getPrenom().toUpperCase() + " a bien change!";
                    tray.setTitle(title);
                    tray.setMessage(message);
                    tray.setNotificationType(NotificationType.SUCCESS);
                    tray.setAnimationType(AnimationType.SLIDE);
